...
const UpdateProjectInfo = () => {
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setDeparmentName(p.deparmentName || "");
        setAcademicYear(p.academicYear || "");
        setAcademicSession(p.academicSession || "");
        setTeachers(p.teachers || []);
        setClassesCount(p.classes?.length || 1);
        setTimeSlots(
          p.timeSlots?.length ? p.timeSlots : [{ from: "", to: "" }],
        );
        setLabs(p.labs || []);
        setLectureHalls(p.lectureHalls || []);
        setRecessSlots(p.recessSlots || []);
        setSessionTypes(
          p.sessionTypes || {
            lectureDuration: 1,
            tutorialDuration: 1,
        ...
        const mappedClasses = p.classes.map((cls) => ({
          ...cls,
          semester: cls.semester || "",
          batches: cls.batches || [],
          subjects: cls.subjects.map((sub) => ({
            ...sub,
            assignedTeachers: sub.assignedTeachers || {
              lectureTeacher: "",
              practicalTeacher: "",
              tutorialTeacher: "",
            },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // --- CRITICAL FIX: Sanitize any invalid legacy IDs before payload construction ---
      ...
      const sanitizedLabs = labs.map(l => {
        if (!isValidId(l._id)) {
          const newId = makeObjectId();
          idMap[l._id] = newId;
          return { ...l, _id: newId };
      ...
      const sanitizedHalls = lectureHalls.map(h => {
        if (!isValidId(h._id)) {
          const newId = makeObjectId();
          idMap[h._id] = newId;
          return { ...h, _id: newId };
      ...
      const fixId = (id) => (idMap[id] ? idMap[id] : id);

      const sanitizedClasses = classesData.map(cls => ({
        ...cls,
        lectureHallId: fixId(cls.lectureHallId),
        subjects: (cls.subjects || []).map(sub => ({
          ...sub,
          labRoom: fixId(sub.labRoom),
          assignedTeachers: {
            lectureTeacher: fixId(sub.assignedTeachers?.lectureTeacher),
            practicalTeacher: fixId(sub.assignedTeachers?.practicalTeacher),
            tutorialTeacher: fixId(sub.assignedTeachers?.tutorialTeacher),
          },
          practicalTeachers: (sub.practicalTeachers || []).map(p => ({
            ...p,
            teacherId: fixId(p.teacherId),
            labId: fixId(p.labId)
          })),
          tutorialTeachers: (sub.tutorialTeachers || []).map(p => ({
            ...p,
            teacherId: fixId(p.teacherId),
            labId: fixId(p.labId)
          }))
      ...
      const payload = {
        ...
        academicSession,
        labs: sanitizedLabs,
        lectureHalls: sanitizedHalls,
        timeSlots,

        totalClasses: Number(classesCount),
        teachers: sanitizedTeachers.map((t) => ({
          teacherName: t.teacherName,
          shortName: t.shortName,
          _id: t._id,
          locationLabId: fixId(t.locationLabId) || undefined,
        })),
        recessSlots,
        sessionTypes,
        user: {
        ...
        classes: sanitizedClasses.map((cls) => ({
          className: cls.className,
          semester: cls.semester || "I",
          saturdayProject: cls.saturdayProject || false,
          batches: cls.batches || [],
          lectureHallId: isValidId(cls.lectureHallId) ? cls.lectureHallId : undefined,
          subjects: cls.subjects.map((sub) => {
            const teachersData = {};
            ...
            const practicalTeachers = (sub.practicalTeachers || [])
              .map((p) => ({
                ...
                labId: isValidId(p.labId) ? p.labId : undefined, // ✅ ADD THIS
              }))
      ...
      const { data } = await axios.put(
        `${config.API_BASE_URL}/ac/update/${id}`,
        payload,
      );
      if (data.success) {
        toast.success("Project updated successfully!");
        navigate(`/project/academic/view/${id}`);
      }
    } catch (error) {
      console.error("Update Error Details:", error.response?.data);
      const errorMsg =
        error.response?.data?.message || "Teacher validation failed.";
  ...
  if (loading)
    return <p className="text-center mt-5">Loading project data...</p>;
  return (
    <Layout>
      <div className="container form-page-background">
        <div className="card shadow-lg border-info rounded-4 p-4">
          <form onSubmit={handleSubmit}>
            <div className=" mb-5 text-center">
              <span
                className="badge rounded-pill bg-primary px-4 py-2 shadow-sm"
            ...
            {classesData.map((cls, cIdx) => (
              <div
                key={cIdx}
                className="border rounded-4 p-4 mb-4 bg-light shadow-sm border-2"
              >
            ...
            {classesData.map((cls, cIdx) => (
              <div
                key={cIdx}
                className="border rounded-4 p-4 mb-4 bg-light shadow-sm"
              >
                ...
                {cls.subjects?.map((sub, sIdx) => (
                  <div
                    key={sIdx}
                    className="row g-2 mb-2 bg-white p-2 rounded shadow-sm align-items-center border mx-0"
                  >
                    <div className="col-md-2">
                      <input
                        type="text"
                      ...
                      <div className="col-12 mt-2">
                        <div className="row g-2 align-items-center">
                          <div className="col-auto">
                            <label className="small text-muted mb-1 d-block">P</label>
                            <input
                              type="number"
                              className="form-control form-control-sm"
                              ...
                              onChange={(e) => {
                                const d = [...classesData];
                                d[cIdx].subjects[sIdx].practicalsPerWeek = Number(e.target.value);
                                setClassesData(d);
                              }}
                            />
                          </div>
                          <div className="col-auto">
                             <label className="small text-muted mb-1 d-block">Prac Teacher</label>
                             <select 
                                className="form-select form-select-sm"
                                value={sub.assignedTeachers?.practicalTeacher || ""}
                                onChange={(e) => {
                                   const d = [...classesData];
                                   d[cIdx].subjects[sIdx].assignedTeachers.practicalTeacher = e.target.value;
                                   setClassesData(d);
                                }}
                             >
                          ...
                          <div className="col-md-12 mt-2">
                            <label className="small text-muted mb-1 d-block">Lab / Room (Common)</label>
                            <select
                              className="form-select form-select-sm"
                              value={sub.labRoom || ""}
                              onChange={(e) => {
                        ...
                        {cls.batches && cls.batches.length > 0 && (
                          <div className="mt-2">
                            <div className="small text-muted mb-1">Per-batch practical teachers & labs</div>
                            <div className="d-flex flex-wrap gap-2">
                              {cls.batches.map((bt, bIdx) => {
                                const existing = sub.practicalTeachers?.find(p => p.batchIndex === bIdx) || {};
                                return (
                                  <div key={bIdx} className="input-group input-group-sm" style={{width: 300}}>
                                    ...
                                    <select 
                                      ...
                                      onChange={(e) => {
                                        const d = [...classesData];
                                        if(!d[cIdx].subjects[sIdx].practicalTeachers) d[cIdx].subjects[sIdx].practicalTeachers = [];
                      ...
                      <div className="col-12 mt-3 border-top pt-2">
                        <div className="row g-2 align-items-center">
                          <div className="col-auto">
                            <label className="small text-muted mb-1 d-block">L</label>
                            <input
                              type="number"
                              className="form-control form-control-sm"
                              ...
                              onChange={(e) => {
                                const d = [...classesData];
                                d[cIdx].subjects[sIdx].lecturesPerWeek = Number(e.target.value);
                                setClassesData(d);
                              }}
                            />
                          </div>
                          <div className="col-auto">
                             <label className="small text-muted mb-1 d-block">Lec Teacher</label>
                             <select 
                                className="form-select form-select-sm"
                                value={sub.assignedTeachers?.lectureTeacher || ""}
                                onChange={(e) => {
                                   const d = [...classesData];
                                   d[cIdx].subjects[sIdx].assignedTeachers.lectureTeacher = e.target.value;
                                   setClassesData(d);
                                }}
                             >
                                <option value="">-- select --</option>
                          ...
                          <div className="col-auto">
                            <label className="small text-muted mb-1 d-block">T</label>
                            <input
                              type="number"
                              className="form-control form-control-sm"
                              ...
                              onChange={(e) => {
                                const d = [...classesData];
                                d[cIdx].subjects[sIdx].tutorialsPerWeek = Number(e.target.value);
                                setClassesData(d);
                              }}
                            />
                          </div>
                          <div className="col-auto">
                             <label className="small text-muted mb-1 d-block">Tut Teacher</label>
                             <select 
                                className="form-select form-select-sm"
                                value={sub.assignedTeachers?.tutorialTeacher || ""}
                                onChange={(e) => {
                                   const d = [...classesData];
                                   d[cIdx].subjects[sIdx].assignedTeachers.tutorialTeacher = e.target.value;
                                   setClassesData(d);
                                }}
                             >
                                <option value="">-- select --</option>
                          ...
                          <div className="col-auto">
                             <button
                                type="button"
                                className="btn btn-sm text-danger mt-3"
            ...
            <div className="text-center mt-4">
              <button
                ...
                disabled={saving}
              >
                {saving ? "Updating Project..." : "Update Academic Project"}
              </button>
            </div>
          </form>
        </div>
