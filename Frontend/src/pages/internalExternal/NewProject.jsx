...
const CreateProjectInfo = () => {
  ...
  const handleClassNameChange = (i, value) => {
    const updated = [...classesData];
    updated[i].className = value;
  ...
  const removeBatch = (classIndex, batchIndex) => {
    const updated = [...classesData];
    if (!updated[classIndex].batches) return;
  ...
  const addTeacher = (classIndex) => {
    ...
    updated[classIndex].teachers.push({ name: "" }); // ✅ correct
    setClassesData(updated);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${config.API_BASE_URL}/ie/create`, payload, {
        headers: { "Content-Type": "application/json" },
      });

      if (res.data.success) {
        alert("Project created successfully!");
        navigate(`/project/internal-external/view/${res.data.project._id}`);
      }
    } catch (error) {
      console.error(error);
      ...
      <div className="container form-page-background">
        <div className=" border border-2 border-info mt-4 rounded-4 p-4 form-card fade-in-up">
          <form onSubmit={handleSubmit} className="text-center">
            <h3 className="badge bg-warning text-dark form-badge">
                  ...
                  <tr key={ci}>
                    ...
                    <td>
                      {(cls.batches || []).map((b, bi) => (
                        <div key={bi} className="d-flex gap-2 mb-1">
                          <input
                            className="form-control"
                            value={b.batchName}
                            onChange={(e) =>
